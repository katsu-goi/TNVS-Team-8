...
export async function classifyDocumentContent(
  ...
  const userPrompt = JSON.stringify({
    ...
    outputShape: {
      ...
      metadataSuggestions: {
        suggestedTitle: "a concise, descriptive title based on content",
        documentNumber: "reference or document number if present (e.g. HIRNA-FAM-MEMO-2026-014)",
        department: "department name if present",
        classification: "INTERNAL, CONFIDENTIAL, PUBLIC, or RESTRICTED if stated",
        documentDate: "document date if present",
        effectiveDate: "effective date if present",
        retentionCategory: "retention policy category if present",
