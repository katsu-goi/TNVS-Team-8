...
/**
 * Optional real-LLM layer for the context-aware assistant. When a configured
 * AI provider (with a real API key) is available, generates a response from the
 * composed system context. Always falls back gracefully when no provider/key is
 * configured or the call fails - the composed context remains available for the
 * caller to review and the assistant never fabricates an answer it could not
 * produce.
 */
@Component
...
public class AiChatGateway {
    ...
    public String chat(String systemContext, String userMessage, String fallback) {
        return chat(systemContext, userMessage, fallback, null);
    }

    /**
     * Chat routed through the per-module AI configuration. When {@code moduleId}
     * is given, the assigned provider/model for that module is used; otherwise
     * the default provider and its configured model.
     */
    public String chat(String systemContext, String userMessage, String fallback, String moduleId) {
        ...
        try {
            ...
            log.debug("LLM chat unavailable, using graceful fallback: {}", e.getMessage());
        }
        return fallback;
    }
