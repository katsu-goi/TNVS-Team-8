...
function dateTime(value: unknown) { const date = new Date(String(value ?? '')); return Number.isNaN(date.getTime()) ? '—' : date.toLocaleString('en-PH', { timeZone: 'Asia/Manila', dateStyle: 'medium', timeStyle: 'short' }); }
...
export const FacilityWorkspace: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    {tabLoading ? <div className="flex min-h-56 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white text-sm text-slate-500"><Loader2 className="h-5 w-5 animate-spin text-red-700" />Loading {tabs.find((item) => item.id === tab)?.label.toLowerCase()}...</div> : <>
      ...
      {tab === 'spaces' && <section className="space-y-4"><div className="flex items-center justify-between"><div><h2 className="text-base font-bold text-slate-900">Rooms & spaces</h2><p className="mt-1 text-xs text-slate-500">Only spaces linked to this facility are returned.</p></div><button type="button" onClick={() => openSpace()} className="inline-flex items-center gap-2 rounded-xl bg-red-700 px-4 py-2.5 text-xs font-bold text-white"><Plus className="h-4 w-4" />Add space</button></div><DataTable data={spaces} rowKey={(row) => row.id} caption="Facility rooms and spaces" columns={[
        { id: 'space', header: 'Room / space', searchableValue: (row) => `${row.name} ${row.roomNumber}`, cell: (row) => <><p className="font-semibold text-slate-900">{row.name}</p><p className="text-xs text-slate-500">{row.roomNumber}</p></>, sortable: true },
        { id: 'type', header: 'Type', accessor: (row) => pretty(row.type), sortable: true },
        { id: 'location', header: 'Location', accessor: (row) => `${row.building || 'No building'} · ${row.floor || (row.floorNumber == null ? 'No floor' : `Floor ${row.floorNumber}`)}`, sortable: true },
        { id: 'capacity', header: 'Capacity', accessor: (row) => row.capacity, sortValue: (row) => row.capacity, sortable: true, align: 'right' },
        { id: 'status', header: 'Status', accessor: (row) => <DataTableStatusBadge value={row.active ? row.status : 'INACTIVE'} />, searchableValue: (row) => row.active ? row.status : 'INACTIVE', sortable: true },
      ] satisfies DataTableColumn<FacilitySpace>[]} searchableText={(row) => `${row.name} ${row.roomNumber} ${row.type} ${row.building ?? ''} ${row.floor ?? ''} ${row.status}`} searchPlaceholder="Search rooms and spaces…" onRefresh={() => void loadTab('spaces')} onRowClick={openSpace} rowActions={(row) => <button type="button" onClick={() => openSpace(row)} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Edit</button>} emptyTitle="No spaces configured" emptyDescription="Add a room or operational space to this facility." /></section>}
      ...
      {tab === 'activity' && <section className="space-y-4"><div><h2 className="text-base font-bold text-slate-900">Facility activity</h2><p className="mt-1 text-xs text-slate-500">Authorized change events for this facility.</p></div>{records.length === 0 ? <Empty icon={ClipboardList} title="No activity recorded" body="Create, update, status, space, floor-plan, and pin events will appear here." /> : <RecordList records={records} title={(row) => pretty(row.action)} meta={(row) => dateTime(row.created_at)} badge={(row) => pretty(row.severity)} detail={(row) => `${text(row, 'description')} · ${text(row, 'user_full_name', 'user_email')}`} />}</section>}
    </>}
...
const RecordList: React.FC<{ records: WorkspaceRecord[]; title: (row: WorkspaceRecord) => string; meta: (row: WorkspaceRecord) => string; badge: (row: WorkspaceRecord) => string; detail: (row: WorkspaceRecord) => string }> = ({ records, title, meta, badge, detail }) => <DataTable
  data={records}
  rowKey={(row) => row.id}
  caption="Facility workspace records"
  columns={[
    { id: 'record', header: 'Record', accessor: (row) => title(row), searchableValue: (row) => title(row), sortable: true },
    { id: 'detail', header: 'Details', accessor: (row) => detail(row), searchableValue: (row) => detail(row) },
    { id: 'schedule', header: 'Date / metadata', accessor: (row) => meta(row), searchableValue: (row) => meta(row), sortable: true },
    { id: 'status', header: 'Status', accessor: (row) => <DataTableStatusBadge value={badge(row)} />, searchableValue: (row) => badge(row), sortable: true },
  ] satisfies DataTableColumn<WorkspaceRecord>[]}
  searchableText={(row) => `${title(row)} ${detail(row)} ${meta(row)} ${badge(row)}`}
  searchPlaceholder="Search facility records…"
  emptyTitle="No facility records"
/>;
