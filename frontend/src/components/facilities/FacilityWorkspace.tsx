...
const emptySpace: FacilitySpaceInput = { name: '', roomNumber: '', building: null, floor: null, floorNumber: null, capacity: 1, type: 'MEETING_ROOM', status: 'AVAILABLE', description: null, active: true };
...
export const FacilityWorkspace: React.FC = () => {
  ...
  const [spaceForm, setSpaceForm] = useState<FacilitySpaceInput>(emptySpace);
  ...
  const saveSpace = async (event: React.FormEvent) => {
    ...
    try {
      const saved = editingSpace
        ? await facilityManagementService.updateSpace(facilityId, editingSpace.id, spaceForm)
        : await facilityManagementService.createSpace(facilityId, spaceForm);
