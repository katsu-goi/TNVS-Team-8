...
export const FacilitiesDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <DashboardHero title="Facilities Manager" subtitle="Facilities Operations & Reservation Oversight" actions={<>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <DashboardMetricCard label="Active Reservations" value={kpi.activeReservations ?? 0} icon={Calendar} color="text-emerald-600" sub="Currently approved" onClick={() => navigate('/facilities/reservations')} />
        <DashboardMetricCard label="Pending Approvals" value={kpi.pendingApprovals ?? 0} icon={CheckSquare} color={(kpi.pendingApprovals ?? 0) > 0 ? 'text-amber-500' : 'text-slate-400'} sub="Awaiting review" onClick={() => navigate('/facilities/approval')} />
        <DashboardMetricCard label="Available Rooms" value={kpi.availableRooms ?? 0} icon={Building2} color="text-emerald-600" sub="Ready for use" onClick={() => navigate('/facilities/rooms')} />
        <DashboardMetricCard label="Occupied Rooms" value={kpi.occupiedRooms ?? 0} icon={Building2} color={(kpi.occupiedRooms ?? 0) > 0 ? 'text-amber-500' : 'text-slate-400'} sub="Currently in use" onClick={() => navigate('/facilities/rooms')} />
        <DashboardMetricCard label="Rooms Under Maintenance" value={kpi.maintenanceRooms ?? 0} icon={Clock} color={(kpi.maintenanceRooms ?? 0) > 0 ? 'text-rose-500' : 'text-slate-400'} sub="Out of service" onClick={() => navigate('/facilities/rooms')} />
        <DashboardMetricCard label="Total Facility Assets" value={kpi.totalAssets ?? 0} icon={ClipboardList} color="text-blue-500" sub="All equipment" onClick={() => navigate('/facilities/assets')} />
        <DashboardMetricCard label="Asset Utilization Rate" value={kpi.assetUtilizationRate != null ? `${kpi.assetUtilizationRate}%` : 'N/A'} icon={BarChart3} color={(kpi.assetUtilizationRate ?? 0) > 50 ? 'text-emerald-600' : 'text-amber-500'} sub="Current rate" onClick={() => navigate('/facilities/analytics')} />
        <DashboardMetricCard label="Today's Reservations" value={kpi.todaysReservations ?? 0} icon={Calendar} color={(kpi.todaysReservations ?? 0) > 0 ? 'text-emerald-600' : 'text-slate-400'} sub="Scheduled today" onClick={() => navigate('/facilities/reservations')} />
      </div>
