export function cameraFailureMessage(error: unknown, secureContext = window.isSecureContext): string {
  ...
  if (name === 'NotAllowedError' || name === 'SecurityError') return "Camera access is blocked. Enable camera permission in your browser's Site Settings or use Manual Token Entry.";
...
export function cameraStatusForFailure(error: unknown, secureContext = window.isSecureContext): CameraStatus {
