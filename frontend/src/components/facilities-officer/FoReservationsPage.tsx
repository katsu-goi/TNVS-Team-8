    ...
    <div className="space-y-6">
      ...
      {activeTab === 'upcoming' && (
        <div className="card-stat overflow-hidden space-y-4 p-4">
          ...
          <DataTable
            data={reservations.filter(row => row.status === 'APPROVED')}
            rowKey={(row) => row.id}
            caption="Approved upcoming reservations"
            columns={[
              { id: 'id', header: 'Reservation ID', accessor: (row) => row.reservationId, sortable: true },
              { id: 'requester', header: 'Requester', searchableValue: (row) => `${row.requesterName} ${row.requesterEmail}`, cell: (row) => <><p className="font-medium text-slate-900">{row.requesterName}</p><p className="text-xs text-slate-500">{row.requesterEmail}</p></>, sortable: true },
              { id: 'facility', header: 'Facility', accessor: (row) => row.facilityName, sortable: true },
              { id: 'date', header: 'Date', accessor: (row) => row.reservationDate, sortable: true },
              { id: 'time', header: 'Time slot', accessor: (row) => `${row.startTime}–${row.endTime} (${row.durationHours}h)`, sortValue: (row) => row.startTime, sortable: true },
              { id: 'status', header: 'Status', accessor: (row) => <DataTableStatusBadge value={row.status} />, searchableValue: (row) => row.status, sortable: true },
            ] satisfies DataTableColumn<ReservationItem>[]}
            searchableText={(row) => `${row.reservationId} ${row.requesterName} ${row.requesterEmail} ${row.facilityName} ${row.status}`}
            searchPlaceholder="Search upcoming reservations…"
            rowActions={(row) => <div className="flex flex-wrap justify-end gap-1.5"><button type="button" onClick={() => setViewDetailModal(row)} className="rounded-lg bg-slate-100 p-2 text-slate-700 hover:bg-slate-200" title="View details" aria-label={`View ${row.reservationId}`}><Eye className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setEditModal(row)} className="rounded-lg bg-blue-50 p-2 text-blue-700 hover:bg-blue-100" title="Edit reservation" aria-label={`Edit ${row.reservationId}`}><Edit3 className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setEditModal(row)} className="rounded-lg bg-teal-50 p-2 text-teal-700 hover:bg-teal-100" title="Reschedule" aria-label={`Reschedule ${row.reservationId}`}><Calendar className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setEscalateModal(row)} className="rounded-lg bg-amber-50 p-2 text-amber-700 hover:bg-amber-100" title="Escalate to manager" aria-label={`Escalate ${row.reservationId}`}><Send className="h-3.5 w-3.5" /></button></div>}
            emptyTitle="No approved upcoming reservations"
            emptyDescription="Approved reservations will appear here."
          />
        </div>
      )}
